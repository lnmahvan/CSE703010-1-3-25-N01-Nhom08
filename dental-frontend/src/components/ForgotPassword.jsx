...
const ForgotPassword = ({ onBackToLogin }) => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      await axios.post('http://localhost:8000/api/password/forgot/send-otp', { email });
      setStep(2);
    } catch (err) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setError(''); setIsLoading(true);
    try {
      const res = await axios.post('http://localhost:8000/api/password/forgot/verify-otp', { email, otp });
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      await axios.post('http://localhost:8000/api/password/forgot/reset', {
        email,
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
      <div className="max-w-md w-full bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100">
        ...
        <div className="p-8 bg-teal-600 text-white text-center relative">
          <button onClick={onBackToLogin} className="absolute top-6 left-6 text-teal-100 hover:text-white transition-colors">
            <ArrowLeft size={24} />
          </button>
        ...
        <div className="p-8">
            ...
            <form onSubmit={handleResetPassword} className="space-y-5 animate-in slide-in-from-right-4">
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-12 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl outline-none"
          ...
          {step === 4 && (
            <div className="text-center space-y-6 animate-in zoom-in-95">
              ...
              <button onClick={onBackToLogin} className="w-full bg-slate-800 hover:bg-slate-900 text-white font-bold py-4 rounded-2xl shadow-lg transition-all">
                Quay về Đăng nhập
              </button>
            </div>
          )}
