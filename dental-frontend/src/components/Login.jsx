...
const Login = ({ onLoginSuccess }) => {
  ...
  // 3. Submit OTP để lấy Token
  const handleVerifyOtp = async (e) => {
    e.preventDefault();
    setError(''); setIsLoading(true);

    try {
      const response = await axios.post('http://localhost:8000/api/verify-login-otp', {
        user_id: tempUserId,
        otp: otp
      });
        ...
        <div className="p-10">
          ...
          {view === 'login' && (
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="relative my-6"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-slate-200"></div></div><div className="relative flex justify-center text-sm"><span className="px-4 bg-white text-slate-500 font-medium">Hoặc tiếp tục với</span></div></div>
